export default class SortableList {
    ...
    sortable(options) {
        ...
        function dragging(e) {
            if (state.isDragged) {
                ...
                state.oEl = state.oEl = elFromPoint(e.pageX, e.pageY);
    ...
    sortableListsToArray(arr, parentId) {
        arr = arr || [];
        var order = 0;
        const newElement = (this.temp == null) ? this.elements : this.temp;

        Array.from(newElement).forEach(element => {
            element.querySelectorAll(':scope > li').forEach(li => {
                ...
                arr.push(listItem);
                this.temp = li.children('ul,ol');
                this.sortableListsToArray(arr, id);
                order++;
    ...
    sortableListsToHierarchy() {
        ...
        Array.from(newElement).forEach(element => {
            element.querySelectorAll(':scope > li').forEach(li => {
                ...
                arr.push(listItem);
                this.temp = li.children('ul,ol');
                listItem.children = this.sortableListsToHierarchy();
                order++;
            });
        });
        this.temp = null;
    ...
    sortableListsToString(arr, parentId) {
        arr = arr || [];
        ...
        Array.from(newElement).forEach(element => {
            element.querySelectorAll(':scope > li').forEach(li => {
                ...
                this.temp = li.children('ul,ol');
                this.sortableListsToString(arr, matches[2]);
